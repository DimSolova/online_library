...
@pytest.mark.parametrize(
    "user, title, description, isbn, author, status_code",
    [
        (
            UserRole.AUTHOR1.value,
            "Идиот",
            "Книга про идиота",
            "9785389071271",
            "Ф.М. Достоевский",
...
async def test_add_book(user, title, description, isbn, author, status_code, ac):
    ...
    response = await resp_author.post(
        "/books",
        json={
            "author": author,
...
@pytest.mark.parametrize(
    "user, book_id, title, description, isbn, author, status_code",
    [
...
async def test_edit_book(
    ...
    response = await resp_author.put(
        f"/books/{book_id}",
        json={
            "author": author,
            "description": description,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            UserRole.AUTHOR1.value,
            1,
            {"title": "Ещё одно новое название", "description": "Новое описание книги"},
            200,
        ),
...
async def test_partially_edit_book(user, book_id, payload, status_code, ac, db):
    ...
    response = await resp_author.patch(f"/books/{book_id}", json=payload)

    assert response.status_code == status_code


@pytest.mark.parametrize(
    "user, book_id, status_code",
    [
...
@pytest.mark.parametrize("book_id, status_code", [(1, 401), (2, 200)])
